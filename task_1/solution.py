...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = 1.0 * RBF(length_scale=1e-1, length_scale_bounds=(1e-2, 1e3)) + WhiteKernel(noise_level=1e-2, noise_level_bounds=(1e-10, 1e1))
        # Initialize the regressor.
        self.gp = GaussianProcessRegressor(kernel=self.kernel, alpha=0.0, normalize_y=False)
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        predictions = gp_mean + self.y_mean                         # We add back the mean (estimated using the training data).
        predictions = predictions + gp_std*test_x_AREA              # We overestimate the particulate levels in the selected areas.
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        # Undersampling (in order to scale).
        idx = self.rng.choice(np.arange(len(train_y)), size=int(0.4*len(train_y)), replace=False)
        X_train = train_x_2D[idx]
        y_train = train_y[idx]
        # X_train = train_x_2D
        # y_train = train_y

        # Fit the model
        # N.B. We substract the target label mean. After all, we assume the prior's mean to be equal to zero.
        self.gp.fit(X_train, y_train - self.y_mean)
...
def perform_extended_evaluation(model: Model, output_dir: str = '/results'):
    ...
    fig, ax = plt.subplots()
    ax.set_title("Predicted Pollution Levels Across the City Grid")
    im = ax.imshow(predictions, vmin=vmin, vmax=vmax, cmap='viridis')
    cbar = fig.colorbar(im, ax = ax)
    cbar.set_label("Pollution (µg/m³)")

    # Save figure to pdf
    figure_path = os.path.join(output_dir, 'task_1.png')
