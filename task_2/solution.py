...
from util import (calc_calibration_curve, cost_function,
                  draw_reliability_diagram, setup_seeds)
...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.theta_sqr = self._create_weight_copy()

        # Full SWAG
        # TODO(2): create attributes for SWAG-diagonal
        #  Hint: check collections.deque
        self.D_hat = {name: collections.deque(maxlen=self.deviation_matrix_max_rank) for name, _ in self.network.named_parameters()}
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            self.theta_sqr[name] = (self.n * self.theta_sqr[name] + param**2)/(self.n + 1)
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            for (batch_xs,) in loader:
                with torch.no_grad():  # Ensure no gradients are computed
                    log_softmax_output = self.network(batch_xs)
                    softmax_output = torch.nn.functional.softmax(log_softmax_output, dim=1)
                    model_sample_predictions.append(softmax_output)
            
            per_model_sample_predictions.append(torch.cat(model_sample_predictions, dim=0))
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_std = torch.sqrt(self.theta_sqr[name] - self.theta_swa[name]**2) / math.sqrt(2)
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                for i, D_i in enumerate(self.D_hat[name]):
                    sampled_param += (D_i * z_2[i]) / math.sqrt(2 * (self.deviation_matrix_max_rank - 1))
